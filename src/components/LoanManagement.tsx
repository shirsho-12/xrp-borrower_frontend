  ...
  const handleRepayment = async (loanId: string) => {
    ...
    setTimeout(() => {
      setLoans((prev) =>
        prev.map((l) =>
          l.id === loanId
            ? {
                ...l,
                balance: Math.max(0, l.balance - l.monthlyPayment),
                status: l.balance - l.monthlyPayment <= 0 ? "REPAID" : l.status,
              }
            : l
        )
      );
      ...
      toast({
        ...
        description: `Payment of $${loan.monthlyPayment.toLocaleString()} processed for ${
          loan.borrowerName
        }`,
      });
    }, 1000);
